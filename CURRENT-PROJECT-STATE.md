...
- Schema version 5 stores authoritative Customer records with sequential `CUST-######` identities. Primary and alternate mobiles have searchable digit-normalized forms; duplicate primary numbers are blocked by default, while an explicit service flag permits a future shared-family-number decision.
...
- Tailoring statuses are limited to Received, In Progress, Ready and Delivered. Status and each garment's current Tailor assignment are operational and mutable; finalized items, charges and measurement snapshots remain immutable. Tailor assignment history is intentionally not stored.
...
- Successful sign-in opens a compact task-oriented Dashboard while Current Stock remains available at `/`. The Dashboard gives New Sale clear priority, keeps Orders / Collection, Customers, Current Stock and Add Stock visible, and leaves Balances / Payments plus Sales & Receipts as small secondary links; setup remains in the sidebar.
...
- Current Stock with parent-scoped cascading filters, one final Apply filters action, refresh-persistent selections, Product-and-Brand totals, and a server-rendered no-JavaScript fallback.
- Product-and-Brand current totals and exact-variant movement history with rename-safe snapshots.
...
- Schema version 5 (`tailor-assignments-v5`) adds Tailor records and one optional current assignment per finalized garment. Existing verified schema-v4 databases require the explicit, backup-first `upgrade-v5` command; startup never migrates automatically.
...
- Billing isolates Product and tailoring actions so tailoring fields never trigger Product-quantity validation. Every non-final server-rendered Billing action restores the staff member's prior scroll position. Customer selection is confirmed inside the Customer section; price, Product and tailoring confirmations are clearly spaced below the action that produced them, while draft-removal feedback remains in the bill section instead of at the page top. Routine Product selection avoids implementation-specific browser guidance. Product additions still show a compact current-bill summary while retaining the complete bill table. Current-bill Fabric is linked automatically when exactly one eligible Fabric line exists, requires an explicit choice when several exist, and keeps Customer-provided or eligible earlier-bill Fabric available as alternate sources.
...
- Tailors are maintained in one small Setup page and may be made inactive without changing earlier records. Each Billing tailoring line can optionally select a Tailor or add one in an inline dialog; Order Detail supports later assignment and reassignment per garment line. One assignment applies to every piece in that line, so different Tailors use separate lines.
...
- Read-only Sales History and finalized bill pages show combined bill totals. The browser-printable 80mm receipt uses the BILL number as its only customer-facing transaction identity, while the internal TAIL number remains in Orders / Collection. The receipt includes product and tailoring lines, Customer identity, cloth source and promised date while keeping measurement detail off the customer receipt.
- Authenticated navigation prioritizes daily counter work in this order: Dashboard, Billing, Orders / Collection, Customers, Current Stock, Add Stock and Sales. Catalogue, Measurements, Stitching Rates and Tailors remain available in a secondary Setup group, followed by Logout. The desktop sidebar distributes the complete navigation and Logout across the available height without an internal scrollbar. Customer records and balances/payments remain separate pages reached through clear tabs within the Customers area; active states and the collapsible mobile menu are preserved.
...
- A LAN-only read-only Tailor View is available at `/tailor`. It uses a separate numeric PIN configured through `YATHREB_TAILOR_PIN`, searches Customer ID/name/mobile, and shows only each customer's current saved measurements, selected styles and notes. The Tailor session cannot open management pages and has no measurement-writing capability.
...
- Tailoring Orders lists and searches finalized orders by BILL number, TAIL number or Customer identity, filters the controlled status, and shows the current outstanding balance after later payments. Order detail keeps original bill values distinct from current outstanding, provides direct bill, customer-account and Receive payment actions, and allows each garment to be assigned, reassigned or left unassigned. Cloth sources and immutable measurement snapshots remain visible; status changes remain unavailable.
...
- One hundred thirty-one automated test methods are present in source. Test execution remains the user's responsibility.
...
- Tailoring status-transition permissions, delivery completion details, and whether promised dates may ever be omitted. Schema 5 requires a promised date and permits controlled status changes without inventing transition rules.
- Initial PKR stitching-rate amounts for all seven standard garments; schema 5 deliberately seeds none and blocks standard tailoring until each required rate is configured.
