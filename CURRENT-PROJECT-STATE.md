...
- Billing isolates Product and tailoring actions so tailoring fields never trigger Product-quantity validation. Every non-final server-rendered Billing action restores the staff member's prior scroll position. Customer selection is confirmed inside the Customer section; price and Product feedback appears below its action, tailoring feedback appears directly below Add tailoring to bill, and draft-removal feedback remains in the bill section instead of at the page top. Product additions still show a compact current-bill summary while retaining the complete bill table. Current-bill Fabric is linked automatically when exactly one eligible Fabric line exists, requires an explicit choice when several exist, and keeps Customer-provided or eligible earlier-bill Fabric available as alternate sources.
...
- Authenticated navigation prioritizes daily counter work in this order: Dashboard, Billing, Orders / Collection, Customers, Current Stock, Add Stock and Sales. Catalogue, Measurements, Stitching Rates and Tailors remain available in a secondary Setup group, followed by Logout. Customer records and balances/payments remain separate pages reached through clear tabs within the Customers area; active states and the collapsible mobile menu are preserved.
