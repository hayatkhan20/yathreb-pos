...
- Billing isolates Product and tailoring actions so tailoring fields never trigger Product-quantity validation. Every non-final server-rendered Billing action restores the staff member's prior scroll position. Customer selection is confirmed inside the Customer section; price, Product, tailoring and draft-removal feedback appears beside the section where the action occurred instead of at the page top. Product additions still show a compact current-bill summary while retaining the complete bill table. Current-bill Fabric is linked automatically when exactly one eligible Fabric line exists, requires an explicit choice when several exist, and keeps Customer-provided or eligible earlier-bill Fabric available as alternate sources.
