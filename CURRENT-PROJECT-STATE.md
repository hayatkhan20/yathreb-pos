...
- Authenticated management navigation uses a persistent desktop sidebar with Dashboard, Catalogue, Add Stock, Current Stock, Billing, Sales, one Customers entry, Tailoring Orders, Measurements, Stitching Rates, and Logout. Customer records and balances/payments remain separate pages reached through clear tabs within the Customers area; the sidebar marks that whole area active and collapses behind a menu control on small screens.
