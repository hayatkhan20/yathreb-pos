...
- Billing isolates Product and tailoring actions so tailoring fields never trigger Product-quantity validation. Adding a Product never scrolls to the top confirmation, and server-rendered tailoring additions restore the staff member's prior Billing scroll position. Product additions still show the confirmation and compact current-bill summary while retaining the complete bill table. Current-bill Fabric is linked automatically when exactly one eligible Fabric line exists, requires an explicit choice when several exist, and keeps Customer-provided or eligible earlier-bill Fabric available as alternate sources.
