...
- Authenticated management navigation uses a persistent desktop sidebar with Dashboard, Catalogue, Add Stock, Current Stock, Billing, Sales, Customers, Customer Balances, Tailoring Orders, Measurements, Stitching Rates, and Logout; it marks the active area and collapses behind a menu control on small screens.
...
- Measurement screens render the seven authoritative templates, decimal-inch fields, Boolean style checkboxes and notes from backend definitions. They load the latest values, append revisions, provide read-only history, and keep Other / Custom Item descriptions and flexible named measurements separate.
...
- Tailoring Orders lists and searches finalized orders by TAIL number or Customer identity, filters the controlled status, and shows bill/customer links, financial snapshots, cloth sources and immutable measurement snapshots. Status changes remain unavailable.
